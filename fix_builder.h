...
private:
    ...
    char *itoa(int64_t val) {
        return itoa(val,tmp,sizeof(tmp));
    }
...
public:
    ...
    FixBuilder(int maxMessageSize) : maxMessageSize(maxMessageSize), buffer(maxMessageSize*2) {
        reset();
    }
    inline void addField(uint32_t tag,const std::string_view& value) {
        ...
        cp+=len;
        *cp++ = '=';
        if(isBodyLen) bodyLenStart = cp;
        memcpy(cp,value.data(),value.size());
        cp+=value.size();
        if(isBodyLen) bodyLenEnd = cp;
        *cp++ = '\x01';
    ...
    const std::string_view messageView() {
        if(bodyLenStart) {
            // write the body length into the already reserved space, must use addTag(9,"00000"); to reserve
